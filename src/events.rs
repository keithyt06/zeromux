...
mod tests {
    ...
    fn create_then_list_roundtrips() {
        let store = mem_store();
        let created = store.create(req("claude-code", "task_done", Some("sess-1"))).unwrap();
        assert!(created.id.starts_with("evt_"));

        let all = store.list(&EventsQuery::default()).unwrap();
        assert_eq!(all.len(), 1);
    ...
    fn list_filters_by_session_and_agent() {
        let store = mem_store();
        store.create(req("claude-code", "task_done", Some("sess-1"))).unwrap();
        store.create(req("codex", "task_done", Some("sess-2"))).unwrap();
        store.create(req("kiro", "tool_use", Some("sess-1"))).unwrap();
    ...
    fn delete_one_removes_event() {
        let store = mem_store();
        let e = store.create(req("kiro", "task_done", None)).unwrap();
        assert!(store.delete_one(&e.id).unwrap());
        assert!(!store.delete_one(&e.id).unwrap());
        assert_eq!(store.list(&EventsQuery::default()).unwrap().len(), 0);
    }
