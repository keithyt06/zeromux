...
- **工作区改动审查** — 在「工作区改动」标签页查看 Agent 未提交的改动（`git status` + `git diff HEAD`），并可一键把「提交/撤销」指令转发给 Agent。对 git 只读、绝不直接写；敏感目录会被拒绝并从 diff 中过滤
- **卡住浮出** — 运行中的轮次静默超过阈值时，会话列表显示琥珀色圆点（180 秒）；当你不在前台时（10 分钟）推送通知提醒
- **Obsidian 笔记库阅读器** — 管理员专属、只读的 Obsidian vault 浏览器（`--vault-dir`）：目录树、文件名搜索、双链（`[[...]]`）跳转、图片渲染、手机两段式阅读布局。绝不写入
