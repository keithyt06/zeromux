...
### PWA + Web Push (peripheral feature)

- **Push** (`src/push.rs`): turns ZeroMux into an installable PWA (`frontend/public/{sw.js,manifest.json}`, registered at root scope by `main.tsx`) that delivers Web Push notifications when the user is away. A VAPID P-256 keypair is loaded-or-generated once and persisted `0600` at `~/.zeromux/vapid.json` (`load_or_generate_vapid`); subscriptions live in a SQLite-backed `PushStore` scoped per `user_id`. REST surface: `GET /api/push/vapid-key`, `POST /api/push/{subscribe,unsubscribe}` (unsubscribe is owner-scoped via `delete_for_user`). Three trigger kinds — `turn_done` (debounced, fires only when away long enough — `should_push_turn_done`), `run_failed`, and `confirm` (batched per owner) — are emitted from `session_manager.rs` and `scheduled_tasks.rs` via `PushService::send_to_user`. **SSRF is guarded twice**: `endpoint_is_safe` runs at subscribe *and* before every send, and the reqwest client is built with `redirect::Policy::none()` so the guard can't be bypassed by a redirect; `410/404` from the push service prunes the dead subscription. Notification level (`important`/`routine`) is shared frontend↔SW through the `zmx-push` Cache (`levels` key); the SW suppresses `turn_done` when the page is focused and deep-links clicks to `?session=`. Crypto is `web-push-native` (pure-Rust, no system `openssl`).
