...
### 1. 🔍 Explore codebase
```
Task: Map how this codebase handles {{input}}, read-only — change nothing.
...
### 4. 🐛 Fix a bug (root cause)
```
Task: Fix this bug — symptom: {{input}}.
...
```
Task: Review the current diff (git diff against the base branch) for {{input}}, with fresh, skeptical eyes — assume it's wrong until proven otherwise.
