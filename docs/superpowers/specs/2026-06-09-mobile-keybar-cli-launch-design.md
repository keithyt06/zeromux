...
KeyBar 为**单行 6 键**(2026-06-09 第二轮:Enter 键短暂加回后又删除,见下):
...
- `^C`:发 `\x03`,行为不变。
- `claude` `codex` `kiro`:点一下向终端发 `<命令>\r`,在当前 shell 立即启动对应 CLI。

> **命令映射(2026-06-09 修正)**:claude/codex 裸命令即进交互会话;
> 但 **kiro 没有 `kiro` 命令**——真实二进制是 `kiro-cli`,且裸 `kiro-cli` 只打印菜单,
> 交互入口是 **`kiro-cli chat`**。所以 kiro 按钮发 `kiro-cli chat\r`,另两个发 `claude\r` / `codex\r`。
> 命令已配置为最高权限模式,不带其它参数。

## 2026-06-09 第二轮:移动端输入体验重做(PM 决策)

实测反馈两个问题:
1. **Enter 键无用且与发送键重复**:手机端不自动聚焦终端(避免一进会话弹键盘),文字全走 composer;
   KeyBar 的 ⏎ 只往空行发 `\r`,看不到反馈,又和 composer 的绿色发送键功能重叠 → **删除 Enter 键**。
2. **聚焦输入框时绿色发送键消失**:输入框字号 `text-sm`(14px),**iOS Safari 对 <16px 输入框聚焦会自动放大整页**,
   发送键被挤出视口 → 输入框改 **`text-base`(16px)**,iOS 不再放大。

布局改动(`TerminalView.tsx`):
- **composer 常驻**:删掉「打字/收起」toggle 与 `composerOpen` 状态,输入框常驻底部。
- 底部从上到下:终端区(flex-1)→ KeyBar → 常驻 composer → 状态栏。
- **软键盘弹起时隐藏状态栏**:`keyboardOpen`(由 VisualViewport `overlap > 120` 判定)为真时 `work_dir/git` 状态栏 `hidden`,
  把空间让给终端 + 输入框;键盘收起恢复。
- composer 占高变化 → 复用既有 fit effect(依赖从 `composerOpen` 换成 `keyboardOpen`)重算 xterm 尺寸。
